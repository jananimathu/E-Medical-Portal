...
header('location:login.php');
	...
	$target_dir = "img/prescriptions/";
		$target_file = $target_dir .date('Y-M-d_h-m-s'). basename($_FILES["fileToUpload"]["name"]);
		$uploadOk = 1;
		...
		echo '<script type="text/javascript">
			window.alert("Sorry, your file is too large.");</script>';
		$uploadOk = 0;
		}
		...
		&& $imageFileType != "gif" ) {
		echo '<script type="text/javascript">
			window.alert("Sorry, only JPG, JPEG, PNG & GIF files are allowed.");</script>';
		$uploadOk = 0;
		}
		...
		if ($uploadOk == 0) {
		echo '<script type="text/javascript">
			window.alert("Sorry, your file was not uploaded.");</script>';
		...
		foreach($value as $qty=> $val34){
			mysqli_query($con,"insert into orders(userId,productId,quantity,prescription_image) values('".$_SESSION['id']."','$qty','$val34','$target_file')");
...
// mysqli_query($con,"insert into orders(userId,productId,quantity,prescription_image) values('".$_SESSION['id']."','$qty','$val34','$path')");
// header('location:payment-method.php');
// }
}
}
		...
		<tbody>
				<tr>
					<td>
						...
						<input type="file" style="margin:15px 15px 35px 15px;border:0px" name="fileToUpload" required>
