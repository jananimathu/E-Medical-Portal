module('Mouse Navigation 2021', {
    setup: function(){
        /*
            Tests start with picker on March 31, 2021.  Fun facts:

            * February 1, 2021 was on a Wednesday
            * February 29, 2021 was on a Wednesday
            * March 1, 2021 was on a Thursday
            * March 31, 2021 was on a Saturday
        */
        this.input = $('<input type="text" value="31-03-2021">')
...
test('Selecting date resets viewDate and date', function(){
    ...
    target.click();
    datesEqual(this.dp.viewDate, UTCDate(2021, 2, 4))
    datesEqual(this.dp.date, UTCDate(2021, 2, 4))
...
test('Navigating next/prev by month', function(){
    ...
    datesEqual(this.dp.viewDate, UTCDate(2021, 1, 29));
    datesEqual(this.dp.date, UTCDate(2021, 2, 31));
    ...
    datesEqual(this.dp.viewDate, UTCDate(2021, 3, 29));
    datesEqual(this.dp.date, UTCDate(2021, 2, 31));
...
test('Navigating to/from year view', function(){
    ...
    datesEqual(this.dp.viewDate, UTCDate(2021, 2, 31));
    datesEqual(this.dp.date, UTCDate(2021, 2, 31));
    ...
    datesEqual(this.dp.viewDate, UTCDate(2021, 3, 1)); // Apr 30
    datesEqual(this.dp.date, UTCDate(2021, 2, 31));
});
...
test('Navigating prev/next in year view', function(){
    ...
    ok(this.picker.find('.datepicker-months').is(':visible'), 'Month picker is visible');
    equal(this.dp.viewMode, 1);
    equal(this.picker.find('.datepicker-months thead th.datepicker-switch').text(), '2021');
    // Not modified when switching modes
    datesEqual(this.dp.viewDate, UTCDate(2021, 2, 31));
    datesEqual(this.dp.date, UTCDate(2021, 2, 31));
...
test('Selecting date from previous month resets viewDate and date, changing month displayed', function(){
    ...
    equal(this.picker.find('.datepicker-days thead th.datepicker-switch').text(), 'March 2021');

    // Updated internally on click
    target.click();
    equal(this.picker.find('.datepicker-days thead th.datepicker-switch').text(), 'February 2021');
    datesEqual(this.dp.viewDate, UTCDate(2021, 1, 26))
    datesEqual(this.dp.date, UTCDate(2021, 1, 26))
...
test('Selecting date from next month resets viewDate and date, changing month displayed', function(){
    ...
    equal(this.picker.find('.datepicker-days thead th.datepicker-switch').text(), 'April 2021');

    // Updated internally on click
    target.click();
    equal(this.picker.find('.datepicker-days thead th.datepicker-switch').text(), 'May 2021');
    datesEqual(this.dp.viewDate, UTCDate(2021, 4, 5))
    datesEqual(this.dp.date, UTCDate(2021, 4, 5))
