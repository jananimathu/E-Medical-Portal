...
exports.format = {
    ...
    "format multiple with zone" : function (test) {
        ...
        var b = moment('2021-10-08 -1200', ['YYYY-MM-DD HH:mm ZZ', 'YYYY-MM-DD ZZ', 'YYYY-MM-DD']);
        test.equals(b.format('YYYY-MM'), '2021-10', 'Parsing multiple formats should not crash with different sized formats');
        test.done();
    },
    ...
    "toJSON" : function (test) {
        var supportsJson = typeof JSON !== "undefined" && JSON.stringify && JSON.stringify.call,
            date = moment("2021-10-09T21:30:40.678+0100");

        test.expect(supportsJson ? 2 : 1);

        test.equal(date.toJSON(), "2021-10-09T20:30:40.678Z", "should output ISO8601 on moment.fn.toJSON");

        if (supportsJson) {
            test.equal(JSON.stringify({
                date : date
            }), '{"date":"2021-10-09T20:30:40.678Z"}', "should output ISO8601 on JSON.stringify");
        }
    ...
    "toISOString" : function (test) {
        var date = moment.utc("2021-10-09T20:30:40.678");

        test.equal(date.toISOString(), "2021-10-09T20:30:40.678Z", "should output ISO8601 on moment.fn.toISOString");
        test.done();
    },
