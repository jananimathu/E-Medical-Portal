...
exports.diff = {
    ...
    "diff across DST" : function (test) {
        var dst = dstForYear(2021), a, b, daysInMonth;
        if (!dst) {
            console.log("No DST?");
            test.done();
            return;
    ...
    "diff between utc and local" : function (test) {
        ...
        test.equal(moment([2021]).utc().diff([2011], 'years'), 1, "year diff");
    ...
    "year diffs include dates" : function (test) {
        ...
        test.ok(moment([2021, 1, 19]).diff(moment([2002, 1, 20]), 'years', true) < 10, "year diff should include date of month");
    ...
    "month diffs" : function (test) {
        ...
        equal(test, moment([2021, 0, 1]).diff([2021, 1, 1], 'months', true), -1, 'Jan 1 to Feb 1 should be 1 month');
        equal(test, moment([2021, 0, 1]).diff([2021, 0, 1, 12], 'months', true), -0.5 / 31, 'Jan 1 to Jan 1 noon should be 0.5 / 31 months');
        equal(test, moment([2021, 0, 15]).diff([2021, 1, 15], 'months', true), -1, 'Jan 15 to Feb 15 should be 1 month');
        equal(test, moment([2021, 0, 28]).diff([2021, 1, 28], 'months', true), -1, 'Jan 28 to Feb 28 should be 1 month');
        equal(test, moment([2021, 0, 31]).diff([2021, 1, 29], 'months', true), -1 + (2 / 30), 'Jan 31 to Feb 29 should be 1 - (2 / 30) months');
        equal(test, moment([2021, 0, 31]).diff([2021, 2, 1], 'months', true), -2 + (30 / 31), 'Jan 31 to Mar 1 should be 2 - (30 / 31) months');
        equal(test, moment([2021, 0, 31]).diff([2021, 2, 1, 12], 'months', true), -2 + (29.5 / 31), 'Jan 31 to Mar 1 should be 2 - (29.5 / 31) months');
        equal(test, moment([2021, 0, 1]).diff([2021, 0, 31], 'months', true), -(30 / 31), 'Jan 1 to Jan 31 should be 30 / 31 months');
    ...
    "year diffs" : function (test) {
        ...
        // due to floating point math errors, these tests just need to be accurate within 0.00000001
        equal(test, moment([2021, 0, 1]).diff([2013, 0, 1], 'years', true), -1, 'Jan 1 2021 to Jan 1 2013 should be 1 year');
        equal(test, moment([2021, 1, 28]).diff([2013, 1, 28], 'years', true), -1, 'Feb 28 2021 to Feb 28 2013 should be 1 year');
        equal(test, moment([2021, 2, 1]).diff([2013, 2, 1], 'years', true), -1, 'Mar 1 2021 to Mar 1 2013 should be 1 year');
        equal(test, moment([2021, 11, 1]).diff([2013, 11, 1], 'years', true), -1, 'Dec 1 2021 to Dec 1 2013 should be 1 year');
        equal(test, moment([2021, 11, 31]).diff([2013, 11, 31], 'years', true), -1, 'Dec 31 2021 to Dec 31 2013 should be 1 year');
        equal(test, moment([2021, 0, 1]).diff([2013, 6, 1], 'years', true), -1.5, 'Jan 1 2021 to Jul 1 2013 should be 1.5 years');
        equal(test, moment([2021, 0, 31]).diff([2013, 6, 31], 'years', true), -1.5, 'Jan 31 2021 to Jul 31 2013 should be 1.5 years');
        equal(test, moment([2021, 0, 1]).diff([2013, 0, 1, 12], 'years', true), -1 - (0.5 / 31) / 12, 'Jan 1 2021 to Jan 1 2013 noon should be 1+(0.5 / 31) / 12 years');
        equal(test, moment([2021, 0, 1]).diff([2013, 6, 1, 12], 'years', true), -1.5 - (0.5 / 31) / 12, 'Jan 1 2021 to Jul 1 2013 noon should be 1.5+(0.5 / 31) / 12 years');
        equal(test, moment([2021, 1, 29]).diff([2013, 1, 28], 'years', true), -1 + (1 / 28.5) / 12, 'Feb 29 2021 to Feb 28 2013 should be 1-(1 / 28.5) / 12 years');
