...
exports.days_in_month = {
    "days in month" : function (test) {
        ...
        for (i = 0; i < 12; i++) {
            test.equal(moment([2021, i]).daysInMonth(),
                       months[i],
                       moment([2021, i]).format('L') + " should have " + months[i] + " days. (beginning of month " + i + ')');
        }
        for (i = 0; i < 12; i++) {
            test.equal(moment([2021, i, months[i]]).daysInMonth(),
                       months[i],
                       moment([2021, i, months[i]]).format('L') + " should have " + months[i] + " days. (end of month " + i + ')');
        }
        test.done();
    },
