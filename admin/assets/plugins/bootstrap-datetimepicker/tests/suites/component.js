module('Component', {
    setup: function(){
        this.component = $('<div class="input-append date" id="datetimepicker">'+
                                '<input size="16" type="text" value="12-02-2021" readonly>'+
...
test('Component gets date/viewDate from input value', function(){
    datesEqual(this.dp.date, UTCDate(2021, 1, 12));
    datesEqual(this.dp.viewDate, UTCDate(2021, 1, 12));
});
...
test('simple keyboard nav test', function(){
    ...
    datesEqual(this.dp.viewDate, UTCDate(2021, 1, 11));
    datesEqual(this.dp.date, UTCDate(2021, 1, 11));
    // Month not changed
    target = this.picker.find('.datetimepicker-days thead th.switch');
    equal(target.text(), 'February 2021', 'Title is "February 2021"');
    ...
    datesEqual(this.dp.viewDate, UTCDate(2021, 2, 11));
    datesEqual(this.dp.date, UTCDate(2021, 2, 11));
    target = this.picker.find('.datetimepicker-days thead th.switch');
...
test('setValue', function(){
    this.dp.date = UTCDate(2021, 2, 13)
    this.dp.setValue()
    datesEqual(this.dp.date, UTCDate(2021, 2, 13));
    equal(this.input.val(), '13-03-2021');
});

test('update', function(){
    this.input.val('13-03-2021');
    this.dp.update()
    datesEqual(this.dp.date, UTCDate(2021, 2, 13));
});
